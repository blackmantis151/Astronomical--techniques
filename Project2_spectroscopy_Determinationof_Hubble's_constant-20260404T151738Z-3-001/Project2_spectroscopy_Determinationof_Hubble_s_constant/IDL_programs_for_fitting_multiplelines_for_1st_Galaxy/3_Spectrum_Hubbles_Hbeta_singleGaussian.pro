...
condi_bkg=where(wave ge 6460 and wave le 6580)
...
condi=where(wave ge  6615-180.0 and wave le  6615+180.0); 8936.15 is decided based on bestfit
...
device,filename='Spectrum_Hbeta_zoom_Gaussfit.ps',/color
;multiplot,[1,2];,ygap=0.00004

plotsym,0,0.5,/fill
plot, wave, flux-mean_bkg_model, psym=8, color=0, xtitle='Wavelength (Angstrom)', $
      ytitle='Flux', linestyle=0, thick=0.001, xrange=[6615-150,6615+150],/xstyle,yrange=[-1,10],/ystyle

oplot,wave, model-mean_bkg_model, sym=-3, color=2, thick=5

;plotsym,0,0.5,/fill
;plot, wave, flux, psym=8, color=0, xtitle='Wavelength (Angstrom)', $
 ;     ytitle='Flux', linestyle=0, thick=0.001, xrange=[6615-150,6615+150],/xstyle,yrange=[-1,20],/ystyle

;oplot,wave, model, sym=-3, color=2, thick=2

;Halpha
oplot,[plamb(1),plamb(1)],[2,10],linestyle=2,color=4,thick=3
xyouts,plamb(1),7,'Hbeta',charsize=1.2,orientation=90
...
ppt[0].fixed=0
ppt[0].limited(0) =1
ppt[0].limits(0)=1.0;ppt_start(0)-5.00d
ppt[0].limits(1)=5.0;ppt_start(0)+5.00d
;;
ppt[1].fixed=0
ppt[1].limited(0) =1
ppt[1].limits(0)=6605.-10.;ppt_start(1)-0.50D
ppt[1].limits(1)=6605.+10;ppt_start(1)+0.750D
;;
ppt[2].fixed=0
ppt[2].limited(0) =1
ppt[2].limits(0)=1.0;ppt_start(2)-0.5d
...
Ha_redshift=(Ha_shifted-lamb0(1))/lamb0(1)
Velocity=(3e+5)*(Ha_redshift)
print,'Hb-shifted A, Hb A, Hbshifted-Hb A, Vel (km/s), Redshift '
print,Ha_shifted,lamb0(1),Ha_shifted-lamb0(1),Velocity, Ha_redshift
